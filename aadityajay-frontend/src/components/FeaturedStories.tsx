...
export default function FeaturedStories({ articles = featuredArticles }: FeaturedStoriesProps) {
  const [active, setActive] = useState<Category>("All");
  const root = useRef<HTMLElement | null>(null);
  const trackRef = useRef<HTMLDivElement | null>(null);
  const pinRef = useRef<HTMLDivElement | null>(null);

  const activeArticles = articles && articles.length > 0 ? articles : featuredArticles;

  const filtered = useMemo(() => {
    if (active === "All") return activeArticles;
    return activeArticles.filter((a) => a.category === active);
