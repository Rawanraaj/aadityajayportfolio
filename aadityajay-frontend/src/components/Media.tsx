...
export default function Media({ videoList = videos }: MediaProps) {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const featEl = root.current?.querySelector("[data-featured-video]");
      if (featEl) {
        gsap.from(featEl, {
          scale: 1.12,
          duration: 1.6,
      ...
      const gridEl = root.current?.querySelector("[data-support-grid]");
      if (gridEl) {
