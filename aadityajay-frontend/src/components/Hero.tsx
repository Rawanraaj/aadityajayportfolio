...
export default function Hero({ tickerList = tickerItems, stats = heroStats }: HeroProps) {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      const portrait = root.current?.querySelector("[data-portrait]");
      if (portrait) {
        gsap.from(portrait, {
      ...
      const heroFadeElements = root.current?.querySelectorAll("[data-hero-fade]");
      if (heroFadeElements && heroFadeElements.length > 0) {
        gsap.from(heroFadeElements, {
          opacity: 0,
          y: 20,
          duration: 1,
          stagger: 0.1,
