...
async function seed() {
  const adminEmail = process.env.ADMIN_EMAIL || "publickhabar24.admin@gmail.com";
  const adminPass = process.env.ADMIN_PASSWORD || "AadityaAdmin2026!";

  console.log("Signing up/in admin user:", adminEmail);
  ...
  if (signInErr) {
    console.log("SignIn notice:", signInErr.message, "- Trying SignUp...");
    const { data: signUpData, error: signUpErr } = await supabase.auth.signUp({
      email: adminEmail,
      password: adminPass,
      options: { data: { display_name: "Aaditya Ajay (Owner)" } }
    });
    ...
    console.log("SignIn successful! User ID:", signInData.user?.id);
  }
  ...
  if (delErr) console.log("Delete error notice:", delErr.message);

  // Insert 8 curated articles
  ...
  console.log(`\n==================================================`);
  console.log(`SUCCESS! Inserted ${inserted.length} articles into Supabase!`);
  console.log(`==================================================\n`);
  inserted.forEach((a, i) => console.log(`${i+1}. [${a.category}] ${a.title}`));

  // Delete temp scripts
  try {
    ['seed_admin_articles.mjs', 'seed_articles.mjs', 'run_audit.mjs'].forEach(f => {
      if (fs.existsSync(f)) fs.unlinkSync(f);
    });
    console.log("\nTemporary seed scripts cleaned up.");
  } catch (e) {
